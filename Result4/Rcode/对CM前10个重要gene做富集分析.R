...
# 读取基因数据
df <- read.csv("E:/工作/论文/黑色素瘤/code/MelanomaAMCM/MelanomaAMCM/data/result4/shapley解释得到的CM重要的前10个基因.csv")
...
pdf("../figure/CM前10个基因重要基因GO富集气泡图.pdf",  width = 10, height = 10)
dotplot(ego, showCategory = 10, font.size  = 22) + 
  ggtitle("Top 10 important genes for CM") +
  theme(
...
write.csv(ego_df, "../data/result/CM前10个重要基因_GO富集结果.csv", row.names = FALSE)
