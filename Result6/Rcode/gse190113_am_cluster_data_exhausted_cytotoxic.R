...
# 读取数据
data <- read.csv("E:/工作/论文/黑色素瘤/code/MelanomaAMCM/MelanomaAMCM/data/result6/result/gse190113_am_cluster_data.csv")
samples <- data[, 1]
labels <- data[, 2]
expr <- data[, -c(1, 2)]
...
# 保存结果
write.csv(plot_data, "../data/result/gse190113_am_cluster_exhausted_cytotoxic_score.csv", row.names = FALSE)
...
ggsave("../figure/gse190113_am_cluster_状态评分小提琴图.pdf", plot = p, width = 7, height = 7, dpi = 600)
