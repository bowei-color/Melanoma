# -*- coding: utf-8 -*-
"""
Created on Fri Jun 13 19:50:36 2025
...
# plt.savefig("../../figure/result3/model在测试集上的auroc曲线图.pdf", format='pdf', dpi=600, bbox_inches='tight')
plt.show()
...
plt.figure(figsize=(20, 20))
for name in model_names:
    df = pd.read_csv(f"../../data/result3/auroc/roc_internal_{name}_deg.csv")
    fpr, tpr, _ = roc_curve(df["y_true"], df["y_prob"])
    roc_auc = auc(fpr, tpr)
    plt.plot(fpr, tpr, label=f"{name} (AUC = {roc_auc:.3f})", color=colors[i], linewidth=6)
    i = i + 1
...
plt.gca().spines['left'].set_linewidth(8)
plt.legend(loc="lower right", fontsize=60, bbox_to_anchor=(1.2, 0))
# plt.savefig("../../figure/result3/model_deg在测试集上的auroc曲线图.pdf", format='pdf', dpi=600, bbox_inches='tight')
plt.show()


model_name = "LR"

df = pd.read_csv(f"../../data/result3/auroc/roc_internal_{model_name}.csv")
df_deg = pd.read_csv(f"../../data/result3/auroc/roc_internal_{model_name}_deg.csv")
...
plt.gca().spines['left'].set_linewidth(8)
plt.legend(loc="lower right", fontsize=60, bbox_to_anchor=(1.2, 0))
plt.savefig("../../figure/result3/model和model_deg在LR上的auroc曲线图.pdf", format='pdf', dpi=600, bbox_inches='tight')
plt.show()